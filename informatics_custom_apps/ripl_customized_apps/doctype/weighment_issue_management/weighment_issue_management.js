...
frappe.ui.form.on("Weighment Issue Management", {
    ...
    workflow_state: function(frm) { 
        ...
        if (frm.doc.workflow_state === "Approved") {  // Triggers only when moving to Approved
            ...
            if (
                frm.doc.issue === "Wrong Card Weighment(Not Manual)" && 
    ...
    issue(frm) {
        ...
        if(frm.doc.issue=="Wrong Card Weighment(Not Manual)" || frm.doc.issue=="Unlink Old & Link New Delivery Note(Weighment Completed)"){
    ...
    entry_type: function(frm) {
        ...
        if (frm.doc.entry_type === "Outward" && frm.doc.custom_is_manual_weighment1 ==1 && frm.doc.custom_is_in_progress1 == 1) {
            ...
            options = ["Vehicle Number Issue","Wrong Card Weighment(Not Manual)","Unlink Old & Link New Delivery Note(Weighment Completed)"];
        }
        else if(frm.doc.entry_type === "Inward" && frm.doc.custom_is_completed1==1 && frm.doc.custom_is_manual_weighment1==0) {
            options = ["Vehicle Number Issue","Wrong Card Weighment(Not Manual)"];
        }
